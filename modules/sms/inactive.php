...
try {	
	...
	$query = sprintf("select count(*) as cnt from smsreminders WHERE user_id = %s and status = 0 ORDER BY senddate DESC", $colname);
	...
	$query = sprintf("select * from smsreminders WHERE user_id = %s and status = 0 ORDER BY senddate DESC", $colname);
...
<table border="0" cellpadding="5" cellspacing="1" class="table">
  ...
  <tr class="td">
    ...
    <td valign="top" bgcolor="#FFFFFF"><?php if($rec['status']==1) { ?>
      <a href="#" onClick="doAjaxLoadingText('<?php echo HTTPROOT; ?>/modules/sms/inactive.php','GET','statusId=<?php echo $rec['id']; ?>&status=0', '', 'center', 'yes', '', '0');">Make Inactive</a>
      <?php } else { ?>
      <a href="#" onClick="doAjaxLoadingText('<?php echo HTTPROOT; ?>/modules/sms/inactive.php','GET','statusId=<?php echo $rec['id']; ?>&status=1', '', 'center', 'yes', '', '0');">Make Active</a>
      <?php } ?>
      <a href="#" onClick="doAjaxLoadingText('<?php echo HTTPROOT; ?>/modules/sms/inactive.php','GET','delId=<?php echo $rec['id']; ?>', '', 'center', 'yes', '', '0');">Delete</a>    </td>
  </tr>
	<?php } ?>
</table>
