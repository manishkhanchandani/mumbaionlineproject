...
class PaginateIt {
	...
	function GetPageLinks(){
        ...
        if(isset($start)){
            ...
            for($i = $start; $i <= $end; $i ++){
                if($i != $this->currentPage){
                    $strLinks .= '<a href="'.$this->linksHref.'?'.$queryString.($i).'" onClick=\'doAjaxLoadingText("'.$this->linksHref.'","GET","'.$queryString.($i).'", "", "center", "yes", "'.$this->linksHref.$queryString.($i).'", "1"); return false;\'>';
                    $strLinks .= ($i).'</a>'.$this->pageSeparator;
                }
                else {
                    $strLinks .= $i.$this->pageSeparator;
			...
			if(($this->currentPage)!=$pageCount && ($start + $this->linksToDisplay - 1 < $pageCount)!=1) {// added
				$strLinks .= $this->pageSeparator."<a href='".$this->linksHref.'?'.$queryString.($this->currentPage+1)."' onClick=\"doAjaxLoadingText('".$this->linksHref."','GET','".$queryString.($this->currentPage+1)."', '', 'center', 'yes', '".$this->linksHref.$queryString.($this->currentPage+1)."', '1'); return false;\">Next</a>"; // added
			} // added

            if($start + $this->linksToDisplay - 1 < $pageCount){
                $strLinks .= $this->pageSeparator.'<a href="'.$this->linksHref.'?'.$queryString.$pageCount.'" onClick=\'doAjaxLoadingText("'.$this->linksHref.'","GET","'.$queryString.$pageCount.'", "", "center", "yes", "'.$this->linksHref.$queryString.$pageCount.'", "1"); return false;\'>';
                $strLinks .= '...'.$pageCount.'</a>'.$this->pageSeparator;
                
				$strLinks .= "<a href='".$this->linksHref.'?'.$queryString.($this->currentPage+1)."' onClick=\"doAjaxLoadingText('".$this->linksHref."','GET','".$queryString.($this->currentPage+1)."', '', 'center', 'yes', '".$this->linksHref.$queryString.($this->currentPage+1)."', '1'); return false;\">Next</a>".$this->pageSeparator; // added
